...
@tf.function
def quad_cohen_kappa_loss(y_true, y_pred, row_label_vec, col_label_vec, weight_mat, eps=1e-6):
    batch_size, num_classes = tf.shape(y_true)[0], tf.shape(y_true)[1]
    labels = tf.matmul(y_true, col_label_vec)
    weight = tf.pow(tf.tile(labels, [1, num_classes]) - tf.tile(row_label_vec, [batch_size, 1]), 2)
...
class QuadCohenKappaLoss(tf.keras.losses.Loss):
    def __init__(self,
        ...
        self.weight_mat = tf.pow(
            tf.tile(self.row_label_vec, [num_classes, 1]) - tf.tile(self.col_label_vec, [1, num_classes]),
            2)
